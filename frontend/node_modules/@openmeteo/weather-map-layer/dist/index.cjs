...
const fileReader = require("@openmeteo/file-reader");
const COLOR_SCALES = {
  cape: {
    type: "breakpoint",
    unit: "J/kg",
...
const VALID_TILE_SIZES = [64, 128, 256, 512, 1024, 2048];
const DEFAULT_TILE_SIZE = 512;
const VECTOR_TILE_EXTENT$1 = 4096;
const DATA_RELEVANT_PARAMS = /* @__PURE__ */ new Set(["variable"]);
const OM_PREFIX_REGEX = /^om:\/\/([^?]+)(?:\?(.*))?$/;
const TILE_SUFFIX_REGEX = /(?:\/)(\d+)(?:\/)(\d+)(?:\/)(\d+)$/i;
const TIME_SELECTED_REGEX = /([0-9]{2}-[0-9]{2}-[0-9]{2}T[0-9]{2}00)/;
const RESOLVE_DOMAIN_REGEX = /data_spatial\/(?<domain>[^/]+)/;
const DOMAIN_META_REGEX = /(http|https):\/\/(?<uri>[\s\S]+)\/(?<domain>[\s\S]+)\/(?<meta>[\s\S]+).json/;
const TIME_STEP_REGEX = /(?<capture>(current_time|valid_times))(_)?(?<modifier>(\+|-))?(?<amountAndUnit>.*)?/;
const VARIABLE_PREFIX = /(?<prefix>(cloud_cover|dew_point|geopotential_height|precipitation|relative_humidity|snow|soil_moisture|soil_temperature|swell|temperature|vertical_velocity|wind(?!_gusts|_direction)))_/;
const LEVEL_REGEX$1 = /((?<height_level_to>\d+_to_.*)|(?<pressure_level>\d+hPa)|(?<height_level>\d+(m|cm)))(?!_)/;
const LEVEL_PREFIX = /(?<prefix>(cloud_cover|geopotential_height|relative_humidity|soil_moisture|soil_temperature|temperature|vertical_velocity|wind(?!_gusts|_direction)))_/;
...
const resolveClippingOptions = (options, useSAB2 = false) => {
...
const writeGeometry = (feature, pbf) => {
...
const generateGridPoints = (pbf, grid, values, directions, x, y, z, clippingOptions, extent = VECTOR_TILE_EXTENT$1, margin = 0) => {
  const features = [];
  const isInsideClip = createClippingTester(clippingOptions);
  ...
  const clippingBounds2 = clippingOptions?.bounds;
...
const domainStep = (time, timeInterval, direction = "floor") => {
...
const closestModelRun = (time, modelInterval) => {
...
const H_TROPOPAUSE = 11e3;
const T_TROPOPAUSE = 216.65;
const pressureHpaToIsaHeight = (hpa) => {
...
const troposphereHeightFromPressure = (hpa) => {
  const r = hpa / P_0;
  const exponent = GAS_CONSTANT * LAPSE / G_0;
  return T_0 / LAPSE * (1 - Math.pow(r, exponent));
};
const stratosphereHeightFromPressure = (hpa) => {
...
const getOptionalColorScale = (variable, colorScalesSource) => {
...
const getColorScale = (variable, dark, colorScalesSource = COLOR_SCALES_WITH_ALIASES) => {
...
const hasColorVariants = (colors) => {
  return !Array.isArray(colors) && "light" in colors && "dark" in colors;
};
const resolveColorScale = (colorScale, dark) => {
  switch (colorScale.type) {
    case "rgba":
      return colorScale;
    case "breakpoint": {
...
const snapBounds = (viewportBounds) => {
...
let clippingBounds = void 0;
const setClippingBounds = (newClippingBounds) => {
...
exports.currentBounds = void 0;
const updateCurrentBounds = (viewportBounds) => {
  ...
  exports.currentBounds = effectiveBounds;
};
const boundsIncluded = (innerBounds, outerBounds) => {
...
const constrainBounds = (bounds, constraint) => {
...
const createProtocolRegistry = (adapterName) => {
...
const renderInWorker = (tileSize, extracted) => {
...
const defaultVectorStyle = (properties) => {
...
const addLeafletProtocolSupport = (L) => {
  if (!L?.GridLayer) {
    throw new Error(
...
const dataToObjectUrl = async (data) => {
  if (data instanceof ImageBitmap) {
...
const addMapboxProtocolSupport = (mapboxgl) => {
  ...
  class OmRasterSource extends RasterTileSource {
    ...
    loadTile(tile, callback) {
      const rawUrl = typeof tile.tileID?.canonical?.url === "function" ? tile.tileID.canonical.url(this.tiles ?? [], this.scheme ?? "xyz") : "";
...
const addOpenLayersProtocolSupport = (ol) => {
...
class WeatherMapLayerFileReader {
  ...
  constructor(config = {}) {
    ...
    this.cache = config.cache ?? new fileReader.LruBlockCache(64 * 1024, 128);
  }
  async setToOmFile(omUrl) {
    this.dispose();
    const s3Backend = new fileReader.OmHttpBackend({
      url: omUrl,
  ...
  async readWithDerivationRule(variable, rule, ranges, signal) {
    ...
    const readOptions = {
      type: fileReader.OmDataType.FloatArray,
      ranges: readRanges,
...
const defaultResolveDataIdentity = (urlComponents, domainOptions2) => {
  ...
  const mapBounds = exports.currentBounds;
...
exports.COLOR_SCALES_WITH_ALIASES = COLOR_SCALES_WITH_ALIASES;
exports.DATA_RELEVANT_PARAMS = DATA_RELEVANT_PARAMS;
exports.DEFAULT_INTERVAL = DEFAULT_INTERVAL;
exports.DEFAULT_TILE_SIZE = DEFAULT_TILE_SIZE;
exports.DOMAIN_META_REGEX = DOMAIN_META_REGEX;
exports.Fragment = Fragment;
exports.GridFactory = GridFactory;
exports.LEVEL_PREFIX = LEVEL_PREFIX;
exports.LEVEL_REGEX = LEVEL_REGEX$1;
exports.LEVEL_UNIT_REGEX = LEVEL_UNIT_REGEX;
exports.OM_PREFIX_REGEX = OM_PREFIX_REGEX;
exports.RESOLVE_DOMAIN_REGEX = RESOLVE_DOMAIN_REGEX;
exports.TILE_SUFFIX_REGEX = TILE_SUFFIX_REGEX;
exports.TIME_SELECTED_REGEX = TIME_SELECTED_REGEX;
exports.TIME_STEP_REGEX = TIME_STEP_REGEX;
exports.VALID_TILE_SIZES = VALID_TILE_SIZES;
exports.VARIABLE_PREFIX = VARIABLE_PREFIX;
exports.VECTOR_TILE_EXTENT = VECTOR_TILE_EXTENT$1;
exports.WeatherMapLayerFileReader = WeatherMapLayerFileReader;
exports.WorkerPool = WorkerPool;
exports.addLeafletProtocolSupport = addLeafletProtocolSupport;
exports.addMapboxProtocolSupport = addMapboxProtocolSupport;
exports.addOpenLayersProtocolSupport = addOpenLayersProtocolSupport;
exports.boundsIncluded = boundsIncluded;
exports.capitalize = capitalize;
exports.checkAgainstBounds = checkAgainstBounds;
exports.clearBlockCache = clearBlockCache;
exports.clipRasterToPolygons = clipRasterToPolygons;
exports.closestModelRun = closestModelRun;
exports.command = command;
exports.constrainBounds = constrainBounds;
exports.createClippingTester = createClippingTester;
exports.defaultFileReaderConfig = defaultFileReaderConfig;
exports.defaultOmProtocolSettings = defaultOmProtocolSettings;
exports.degreesToRadians = degreesToRadians;
exports.derivative = derivative;
exports.domainGroups = domainGroups;
exports.domainOptions = domainOptions;
exports.domainStep = domainStep;
exports.drawOnTiles = drawOnTiles;
exports.ensureData = ensureData;
exports.fastAtan2 = fastAtan2;
exports.generateArrows = generateArrows;
...
exports.getProtocolInstance = getProtocolInstance;
exports.getRanges = getRanges;
exports.getValueFromLatLong = getValueFromLatLong;
exports.hermite = hermite;
exports.index = index;
exports.interpolate = interpolate;
exports.lat2tile = lat2tile;
exports.levelGroupVariables = levelGroupVariables;
exports.lon2tile = lon2tile;
exports.modPositive = modPositive;
exports.normalizeLon = normalizeLon;
exports.normalizeUrl = normalizeUrl;
exports.omProtocol = omProtocol;
exports.pad = pad;
exports.radiansToDegrees = radiansToDegrees;
exports.ratio = ratio;
exports.resolveClippingOptions = resolveClippingOptions;
exports.resolveColorScale = resolveColorScale;
exports.rotatePoint = rotatePoint;
exports.roundWithPrecision = roundWithPrecision;
exports.secondDerivative = secondDerivative;
exports.setClippingBounds = setClippingBounds;
exports.sharedPolygonsPolygonCount = sharedPolygonsPolygonCount;
exports.sharedPolygonsRing = sharedPolygonsRing;
exports.sharedPolygonsRingCount = sharedPolygonsRingCount;
exports.snapBounds = snapBounds;
exports.tile2lat = tile2lat;
exports.tile2lon = tile2lon;
exports.updateCurrentBounds = updateCurrentBounds;
exports.variableOptions = variableOptions;
exports.writeFeature = writeFeature;
exports.writeGeometry = writeGeometry;
exports.writeLayer = writeLayer;
